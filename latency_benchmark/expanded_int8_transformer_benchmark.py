#!/usr/bin/env python3
"""
Expanded INT8 Transformer Benchmark Script
Tests transformer models with INT8 quantization and higher batch sizes
"""
...
class INT8TransformerBenchmark:
    ...
    def convert_to_int8(self, model, model_name):
        ...
        try:
            ...
            if 'vit' in model_name.lower():
                # For ViT, use image inputs
                calibration_data = torch.randn(100, 3, 224, 224, device=self.device)
            else:
                # For text models, use tokenized inputs
                calibration_data = torch.randint(0, 30522, (100, 512), device=self.device)
    ...
    def benchmark_model(self, model_name, batch_sizes, num_warmup=10, num_runs=50):
        """Benchmark a model with different batch sizes in INT8."""
        print(f"\nBenchmarking {model_name} in INT8...")
        ...
        # Convert to INT8
        int8_model = self.convert_to_int8(model, model_name)
        if int8_model is None:
            print(f"Failed to quantize {model_name}, skipping...")
            return
        ...
        for compilation_mode in ['Eager', 'torch.compile']:
            ...
            if compilation_mode == 'torch.compile':
                try:
                    compiled_model = torch.compile(int8_model, mode='default')
                except Exception as e:
                    print(f"    Compilation failed: {e}, skipping...")
                    continue
                test_model = compiled_model
            else:
                test_model = int8_model
        ...
        del model, tokenizer, int8_model
        self.clear_cache()
    ...
    def save_results(self, filename=None):
        ...
        if filename is None:
            ...
            filename = f"expanded_int8_transformer_benchmark_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(description='Expanded INT8 Transformer Benchmark')
    ...
    benchmark = INT8TransformerBenchmark()
