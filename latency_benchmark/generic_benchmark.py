...
# BetterTransformer is now handled automatically by transformers/PyTorch if available; no manual intervention needed.
DISABLE_CUDA_GRAPHS = False  # Set to True to disable CUDA graphs for testing interference effects
            ...
            if 'inductor' in modes_to_run:
                ...
                try:
                    if DISABLE_CUDA_GRAPHS:
                        # Disable CUDA graphs to test for interference effects
                        compiled_model = torch.compile(base_model, mode="max-autotune")
                        print("    -> Using max-autotune mode (CUDA graphs disabled by default)")
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, choices=['eager', 'torchscript_trace', 'torchscript_script', 'inductor'], 
                       help="Run only specific benchmark mode (default: run all modes)")
    args = parser.parse_args()
    main(args)

'''
ncu -o ncu_inductor_bs1_report --set full python generic_benchmark.py --model=bert-base-uncased --batch_sizes=1 --mode=inductor --iterations=1
ncu -o ncu_eager_bs1_report --set full python generic_benchmark.py --model=bert-base-uncased --batch_sizes=1 --mode=eager --iterations=1
